...
--			ㄴ 사용자(개발자)가 테이블처럼 select를 조회를 할 수 있는 테이블
--			ㄴ grant resource, connect to idev;		-> resource 에 view 생성은 제외입니다.


CREATE VIEW v_buy
AS
SELECT PCODE, CUSTOM_ID FROM tbl_buy#;	--오류 : insufficient privileges(불충분한 권한)
...
--SELECT * FROM tbl_buy# b, tbl_custom#

CREATE VIEW v_sale
AS
SELECT tc.CUSTOM_ID, tc, tc.NAME, tc.EMAIL , tb.QUANTITY, tb.BUY_DATE
