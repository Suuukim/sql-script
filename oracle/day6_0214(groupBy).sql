-- 집계함수(통계함수, 그룹) : count, max, min, sum, avg, 외에 (분산,표준편차,중앙값) 등의 함수도 있음.
...
-- gorup by를 적용하여 업무별로 count, avg 구합니다.
SELECT JOB_ID , COUNT(*),AVG(salary)
FROM EMPLOYEES e 
GROUP BY JOB_ID ;
...
-- departments 테이블과 위의 부서별 집계를 join 
...
SELECT d.DEPARTMENT_ID , DEPARTMENT_NAME , d2."인원수" , d2."평균"  
FROM DEPARTMENTS d ,
		(SELECT DEPARTMENT_ID, count(*) AS "인원수", avg(salary) AS "평균"
		FROM EMPLOYEES e 
		GROUP BY DEPARTMENT_ID) d2
...
-- 			  기존 방법은 amx 값보다 큰것을 찾는 것. -> ALL 키워드로 바꿔보면
